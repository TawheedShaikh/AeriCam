...
def show_alert_box():
    def on_okay():
        ...
        session_name_window.destroy()
        update_video_with_session_name()

    session_name_window = tk.Toplevel(root)
    session_name_window.title("Name Your Session")
    session_name_window.geometry("300x150")

    label = tk.Label(session_name_window, text="Name your session:")
    label.pack(pady=10)

    session_name_entry = tk.Entry(session_name_window)
    session_name_entry.pack(pady=10)

    okay_button = tk.Button(session_name_window, text="Okay", command=on_okay)
    okay_button.pack(pady=10)

    session_name_window.grab_set()  # Make the window modal
...
def add_overlay_text(frame, roll_value):
    ...
    # Add session name to the frame
    if session_name:
        session_name_text = f"Session: {session_name}"
        cv2.putText(frame, session_name_text, (frame.shape[1] - 200, 30), font, 1, (0, 255, 0), 2, cv2.LINE_AA)
...
def start_video_recording_callback():
    ...
    if not recording:
        ...
        show_alert_box()  # Show the alert box after stopping recording
